...
- 操作序列：这种结构将文档内容表示为一系列的操作，每个操作表示对文档内容的修改。通过应用这些操作，可以重现文档的内容。比如 OT（Operational Transformation）和 CRDT（Conflict-free Replicated Data Type）就是基于操作序列的。
...
也就是说，树形结构在视图层面上带来了丰富的表达力，但在操作层面上也带来了复杂性。
